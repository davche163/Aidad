...
log_file = '/home/openailab/log/guard_%s.log' % datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d')
...
def post(server_url, params):
    ...
    try:
        ...
        logging.error("urllib2 request error!")
    try:
...
def process_image(server_url, image_path):
    ...
    try:
        ...
        logger.warning("open mark_file failed! %s",mark_file)
        exceptionFlag = 1
    try:
        proof_file = open(proof_path, "rb")
    except:
        logger.warning("open proof_file failed! %s", proof_file)
        exceptionFlag = 1
    
    if 0 == exceptionFlag :
        #proof_image = pickle.dumps(proof_file.read())
        #mark_image = pickle.dumps(mark_file.read())
    ...
    while 1:
        ...
        for parent, dirnames, filenames in os.walk(os.getcwd()):
            for filename in filenames:
                ...
                if filename.endswith('100_0.99_1.jpg'):
                    logger.info(filename)
                    time.sleep(5)  # ensure file writing completed
