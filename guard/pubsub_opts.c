...
char* readfile(int* data_len, struct pubsub_opts* opts)
{
	...
	buffer = static_cast<char*>(malloc(sizeof(char)*filesize));
	if (buffer == NULL)
	{
...
void logProperties(MQTTProperties *props)
{
	...
	for (i = 0; i < props->count; ++i)
	{
		...
		const char* name = MQTTPropertyName(static_cast<MQTTPropertyCodes>(id));
		char* intformat = "Property name %s value %d\n";

		switch (MQTTProperty_getType(static_cast<MQTTPropertyCodes>(id)))
		{
